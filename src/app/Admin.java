...
public final class Admin {
    ...
    public void setUsers(final List<UserInput> userInputList) {
        ...
        for (UserInput userInput : userInputList) {
            users.add((User) userFactory.getUser("user", userInput.getUsername(),
    ...
    public String addNewUser(final CommandInput commandInput) {
        ...
        UserFactory userFactory = UserFactory.getInstance();
        UserAbstract userAbstract = userFactory.getUser(type, username, age, city);

        if (type.equals("user")) {
            users.add((User) userAbstract);
        } else if (type.equals("artist")) {
            artists.add((Artist) userAbstract);
        } else {
            hosts.add((Host) userAbstract);
        }
    ...
    public String addAlbum(final CommandInput commandInput) {
        ...
        currentArtist.notifyObservers(new Notification("New Album", "New Album from " + currentArtist.getUsername() + "."));
    ...
    public String addPodcast(final CommandInput commandInput) {
        ...
        currentHost.notifyObservers(new Notification("New Podcast", "New Podcast from " + currentHost.getUsername() + "."));
    ...
    public String addEvent(final CommandInput commandInput) {
        ...
        currentArtist.notifyObservers(new Notification("New Event", "New Event from " + currentArtist.getUsername() + "."));
    ...
    public String addMerch(final CommandInput commandInput) {
        ...
        currentArtist.notifyObservers(new Notification("New Merchandise", "New Merchandise from " + currentArtist.getUsername() + "."));
    ...
    public String addAnnouncement(final CommandInput commandInput) {
        ...
        currentHost.notifyObservers(new Notification("New Announcement", "New Merchandise from " + currentHost.getUsername() + "."));
    ...
    public String subscribe(final String name) {
        if (Admin.getInstance().getUser(name) == null) {
            return "The username " + name + " doesn't exist.";
        }

        ContentCreator contentCreator = (ContentCreator) Admin.getInstance().getUser(name).getCurrentPage().getPerson();

        if (!(contentCreator.userType().equals("host") || contentCreator.userType().equals("artist"))) {
            return "To subscribe you need to be on the page of an artist or host.";
        }


        if (contentCreator.observerExists(Admin.getInstance().getUser(name))) {
            // Admin.getInstance().getUser(name).getCurrentPage().getPerson().addSubscriber(Admin.getInstance().getUser(name));
            contentCreator.unregisterObserver(Admin.getInstance().getUser(name));
            return name + " unsubscribed from " + Admin.getInstance().getUser(name).getCurrentPage().getPerson().getUsername() + " successfully.";
        } else {
            // Admin.getInstance().getUser(name).getCurrentPage().getPerson().removeSubscriber(Admin.getInstance().getUser(name));
            contentCreator.registerObserver(Admin.getInstance().getUser(name));
            return name + " subscribed to " + Admin.getInstance().getUser(name).getCurrentPage().getPerson().getUsername()+ " successfully.";
        }

    }

    public List<Notification> getNotifications(CommandInput command) {
